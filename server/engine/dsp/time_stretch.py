...
class TimeStretcher:
    ...
    def apply_pitch_shift(self, audio, n_steps):
        """
        [New] Pitch Shifting
        오디오의 길이는 유지하고, 음정(Pitch)만 n_steps 만큼 바꿉니다.
        :param n_steps: 반음 단위 (예: 1.0 = 반음 올림, -2.0 = 온음 내림)
        """
        if len(audio) == 0: return audio
        if n_steps == 0: return audio.copy()
        
        # librosa.effects.pitch_shift는 꽤 느린 편이지만 품질은 준수합니다.
